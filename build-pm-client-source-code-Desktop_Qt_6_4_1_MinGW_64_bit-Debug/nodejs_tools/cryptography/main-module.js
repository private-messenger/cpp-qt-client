...
class Crypt {
	...
	encrypt (data) {
		...
		let toEncrypt = data;
		let encryptBuffer = Buffer.from(toEncrypt);
...
module.exports = {
	...
	CryptKey : CryptKey,
	md5      : md5_gen,
	generate : (keySize = 1024, passphrase = null) => {
		let keysObj;
	...
	generateAES: (length = 32) => {
		if ([24, 32, 16].indexOf(length) == -1) throw new CryptorException('AES key must be 16, 24 or 32 bytes.');
