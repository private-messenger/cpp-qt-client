...
char* md5FromData (char* data) {
    std::string scriptPath = "nodejs_tools/cryptography/md5-gen.js ";
    std::string dataStr = data;
    std::string command = scriptPath + dataStr;
    qDebug(command.c_str());
    return nodejsRun((char*)command.c_str()).data();
    //return "";
}

void DatabaseAppInterface::setValue (char* key, char* value) {
    sqlite3_open("appdata", &database);
...
unsigned char* generateAESkey () {
    std::string cmd("nodejs_tools/cryptography/aes-keygen.js");
    return (unsigned char*)nodejsRun((char*)((cmd).c_str())).data();
}

bool DatabaseAppInterface::reg (std::string login, std::string password) {
    /*char* id = generateId();  // Ps: это лучшее, что я мог сделать
    ...
    if (rc != SQLITE_OK) {
        ...
        unsigned char* logPasKey = (unsigned char*)md5FromData((char*)((login + ":" + password).c_str()));
        AES256* encoding = new AES256((unsigned char*)logPasKey);
        AES256* storage = new AES256(generateAESkey());
        RSA* rsaKeys = new RSA(NULL, NULL);
        // Шифруем ключи, пароль
        //char* passwordEncoded = (char*)logPasKey;
        char* passwordEncoded = (char*)encoding->encode((unsigned char*)logPasKey);
